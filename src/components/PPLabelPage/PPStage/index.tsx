...
const Component: ForwardRefRenderFunction<pageRef, PPStageProps> = (props, ref) => {
  ...
  const renderShape = () => {
    ...
    const index = annos.findIndex(
      (item) => item.annotationId === props.currentAnnotation?.annotationId,
    ); // 找到数组中 id 相等的对象的下标
    if (index !== -1) {
      // 如果找到了
      annos.push(annos.splice(index, 1)[0]); // 将找到的对象从原来的位置转移到数组最后一位
    }
  ...
  const onMouseDown = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (e.evt.button === 1) {
      if (props?.tool?.curr === 'polygon') {
        ...
        return;
      } else if (props?.tool?.curr === 'interactor') {
        onMousepoint();
        return;
      } else {
        props.changePreTools(props?.tool?.curr);
        props.tool.setCurr('mover');
      }
    }
    setisClick(true);
    ...
    if (props?.tool?.curr === 'polygon' && ctx) {
      if (e.evt.button === 2) {
        ...
        if (
          ...
          props.annotationDelete(newAnnotaions, true);
        } else {
      ...
      if (props.currentTool === 'editor') {
        console.log('props.currentTool', props.currentAnnotation);
        const annos = props.annotations?.filter((anno) => {
          if (anno.annotationId !== props.currentAnnotation?.annotationId) {
            return anno;
          }
        });
        let resulsts: any = [];
    ...
    >
      ...
      <Stage
        ...
        onDragEnd={(evt) => {
          ...
          //   return;
          // }
          console.log('evt.target.x()', evt.target.x(), evt.target.y());
